    ...
    <Card class="w-full max-w-sm shadow-card">
      <CardContent class="pt-10 pb-10 px-8">
        ...
        <form @submit.prevent="handleSubmit">
          <div class="mb-4">
            <Input v-model="password" type="password" :placeholder="t('RESET_PASSWORD.PASSWORD')" required
              minlength="6" />
          </div>
          <div class="mb-4">
            <Input v-model="passwordConfirmation" type="password" :placeholder="t('RESET_PASSWORD.CONFIRM_PASSWORD')"
          ...
          <p class="text-center text-sm text-muted-foreground mt-5 mb-0">
            <a href="#" @click.prevent="router.push({ name: 'login' })"
              class="text-primary font-semibold no-underline hover:underline">{{ t('RESET_PASSWORD.BACK') }}</a>
          </p>
        </form>
      </CardContent>
    </Card>
