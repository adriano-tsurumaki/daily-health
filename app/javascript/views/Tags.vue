...
<template>
  <div>
    ...
    <div v-else class="space-y-3">
      <div v-for="tag in store.tags" :key="tag.id" class="flex items-center gap-3 p-3 border rounded-md">
        ...
        <template v-else>
          ...
          <div class="flex-1" />
          <Button variant="link" class="p-0 h-auto" @click="startEdit(tag.id, tag.name)">{{ t('FINANCE.EDIT') }}</Button>
          <Button variant="link" class="p-0 h-auto text-destructive" @click="handleDelete(tag.id)">{{ t('FINANCE.DELETE') }}</Button>
        </template>
      </div>
    </div>
  </div>
