...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <Button as-child>
        <RouterLink :to="{ name: 'finance-transactions-new' }">{{ t('FINANCE.NEW_TRANSACTION') }}</RouterLink>
      </Button>
    </div>
    ...
    <div v-if="store.dashboard" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      <Card class="p-6">
        ...
        <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ formatCurrency(store.dashboard.total_income) }}</p>
      </Card>
      <Card class="p-6">
        <p class="text-sm text-muted-foreground mb-1">{{ t('FINANCE.EXPENSE') }}</p>
        <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ formatCurrency(store.dashboard.total_expense) }}</p>
      </Card>
      <Card class="p-6">
        <p class="text-sm text-muted-foreground mb-1">{{ t('FINANCE.BALANCE') }}</p>
        <p class="text-2xl font-bold" :class="store.dashboard.balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
    ...
    <div v-else-if="!store.dashboard?.recent_transactions?.length" class="text-center py-8 text-muted-foreground">
      ...
      <Table>
        ...
        <TableBody>
          <TableRow v-for="tx in store.dashboard.recent_transactions" :key="tx.id">
            ...
            <TableCell class="text-right" :class="tx.transaction_type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
              {{ formatCurrency(tx.amount) }}
            </TableCell>
            <TableCell class="text-right">
              <Button variant="link" as-child class="p-0 h-auto">
                <RouterLink :to="{ name: 'finance-transactions-edit', params: { id: tx.id } }">{{ t('FINANCE.EDIT') }}</RouterLink>
              </Button>
            </TableCell>
          </TableRow>
        </TableBody>
